...
#| warning: false
ggplot(datos_diabetes) +
  geom_point(aes(x = (weight * 0.453592) / (height * 0.0254)^2, y = chol / hdl), color = "purple", size = 0.5) +
  xlab("IMC") +
  ylab("Radio de colesterol") +
  ggtitle("Relación entre IMC y colesterol") +
  theme_bw()
```
...
plot_grid(
  ggplot(jurisdicciones) +
    geom_point(aes(x = poblacion, y = casos, color = "red")) +
    labs(title = "c) color DENTRO del aes") +
    theme_bw(),
  ggplot(jurisdicciones) +
    geom_point(aes(x = poblacion, y = casos), color = "red") +
    labs(title = "d) color FUERA del aes") +
    theme_bw(),
  ncol = 2
)
```
...
ggplot(jurisdicciones) +
  geom_point(aes(x = poblacion, y = casos)) +
  ylim(0, 20) +
  labs(title = "e) ¡Se perdieron 3 puntos!") +
  theme_bw()
```
:::
...
```{r}
orden <- c(
  "Estados Unidos Mexicanos", "Aguascalientes", "Baja California",
...
ggplot(dosis_aplicadas) +
  geom_col(aes(x = jurisdiccion, y = dosis)) +
  labs(title = "a) Ojo: van en orden alfabético") +
  theme_bw()
```
...
ggplot(dosis_aplicadas) +
  geom_col(aes(x = dosis, y = jurisdiccion)) +
  labs(title = "b) Barras horizontales") +
  theme_bw()
```
...
ggplot(dosis_aplicadas) +
  geom_bar(aes(x = jurisdiccion)) +
  labs(title = "c) geom_bar cuenta filas: todas miden 1") +
  theme_bw()
```
...
ggplot(dosis_aplicadas) +
  geom_col(aes(x = "", y = dosis, fill = jurisdiccion)) +
  labs(title = "d) Una sola barra apilada", x = "") +
  theme_bw()
```
:::
...
ggplot(datos_diabetes) +
  ...
  annotate("label",
    x = promedio_colesterol, y = 0.0001, label = "Media = 208",
    color = "tomato3"
  ) +
  annotate("label",
    x = q025, y = 0.0001, label = glue::glue("2.5% = {q025}"),
    color = "tomato3"
  ) +
  annotate("label",
    x = q975, y = 0.0001, label = glue::glue("97.5% = {q975}"),
    color = "tomato3"
  ) +
...
plotA <- ggplot(mis.datos) +
  geom_histogram(aes(x = A, y = (after_stat(count)) / sum(after_stat(count))), fill = "tomato3", color = "black", binwidth = bw) +
  ylab("Proporción") +
  theme(
    plot.background = element_rect(fill = "#fffff8"),
    panel.background = element_rect(fill = "#fffff8")
...
plotB <- ggplot(mis.datos) +
  geom_histogram(aes(x = B, y = (after_stat(count)) / sum(after_stat(count))), fill = "forestgreen", color = "black", binwidth = bw) +
  theme(
    plot.background = element_rect(fill = "#fffff8"),
    panel.background = element_rect(fill = "#fffff8")
  ) +
...
plotC <- ggplot(mis.datos) +
  geom_histogram(aes(x = C, y = (after_stat(count)) / sum(after_stat(count))), fill = "purple", color = "black", binwidth = bw) +
  ylab("Proporción") +
  theme(
    plot.background = element_rect(fill = "#fffff8"),
    panel.background = element_rect(fill = "#fffff8")
...
plotD <- ggplot(mis.datos) +
  geom_histogram(aes(x = D, y = (after_stat(count)) / sum(after_stat(count))), fill = "deepskyblue3", color = "black", binwidth = bw) +
  ylab("Proporción") +
  theme(
    plot.background = element_rect(fill = "#fffff8"),
    panel.background = element_rect(fill = "#fffff8")
...
plot_grid(
  ggplot(brote) +
    geom_line(aes(x = semana, y = casos)) +
    labs(title = "a) sólo línea") +
    theme_bw(),
  ggplot(brote) +
    geom_line(aes(x = semana, y = casos)) +
    geom_point(aes(x = semana, y = casos)) +
    labs(title = "b) línea + puntos") +
    theme_bw(),
  ncol = 2
)
```
...
plot_grid(
  ggplot(brote_desordenado) +
    geom_line(aes(x = semana, y = casos)) +
    labs(title = "c) geom_line: ordena por x") +
    theme_bw(),
  ggplot(brote_desordenado) +
    geom_path(aes(x = semana, y = casos)) +
    labs(title = "d) geom_path: respeta el orden") +
    theme_bw(),
  ncol = 2
)
```
