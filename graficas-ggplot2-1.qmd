...
aliases:
  - "/Gráficas_con_ggplot2_parte_1.html"
---
...
::: callout-note
Los datos **no están en el repositorio**; los descargas de [Dropbox](https://www.dropbox.com/sh/ajgteruqo1wf47d/AADMfTnIUeIFSn_3whMVYgjfa?dl=0) y los descomprimes en una carpeta `datasets/` junto a tu script (ver la [presentación](index.qmd#los-datos)).
:::
...
paquetes](introduccion-a-r.qmd#instalaci%C3%B3n-de-paquetes)
...
### Ejercicio a mano: dibuja la gráfica antes de correrla
...
plot_grid(
  ...
  ggplot(jurisdicciones) + geom_point(aes(x = casos, y = poblacion)) +
    labs(title = "b)") + theme_bw(),
  ncol = 2)
```
...
#| echo: false
plot_grid(
  ggplot(jurisdicciones) + geom_point(aes(x = poblacion, y = casos, color = "red")) +
    labs(title = "c) color DENTRO del aes") + theme_bw(),
  ggplot(jurisdicciones) + geom_point(aes(x = poblacion, y = casos), color = "red") +
    labs(title = "d) color FUERA del aes") + theme_bw(),
  ncol = 2)
```
...
#| warning: false
ggplot(jurisdicciones) + geom_point(aes(x = poblacion, y = casos)) +
  ylim(0, 20) + labs(title = "e) ¡Se perdieron 3 puntos!") + theme_bw()
```
:::
...
a.  Falta el `+` entre capas. En `ggplot2` **cada capa se pega con un `+`**. Correcto: `ggplot(jurisdicciones) + geom_point(...)`.

b.  El `+` quedó al **inicio** del renglón. `R` lee el primer renglón, ve que ya está completo y lo ejecuta (te dibuja el lienzo vacío); luego lee el segundo y no entiende qué le estás sumando. El `+` **siempre va al final del renglón anterior**, nunca al principio del siguiente.

c.  Falta el `aes`. Sin él, `ggplot2` no sabe que `poblacion` y `casos` son columnas de la base y las busca como si fueran variables sueltas del ambiente. Correcto: `geom_point(aes(x = poblacion, y = casos))`.

d.  Mayúsculas. Las columnas se llaman `poblacion` y `casos`, no `Poblacion` ni `Casos`. Si dudas de cómo se llaman exactamente, escribe `names(jurisdicciones)`.

e.  Punto y coma en lugar de coma. Los argumentos de una función se separan **con coma**.

f.  Los argumentos van al revés. En los `geom_` el primer argumento es el `mapping` (el `aes`), no los datos, así que hay que nombrar el de datos: `geom_point(data = jurisdicciones, aes(x = poblacion, y = casos))`.
:::
...
### Ejercicio a mano: dibuja las barras
...
```{r}
#| echo: false
ggplot(dosis_aplicadas) + geom_col(aes(x = jurisdiccion, y = dosis)) +
  labs(title = "a) Ojo: van en orden alfabético") + theme_bw()
```
...
```{r}
#| echo: false
ggplot(dosis_aplicadas) + geom_col(aes(x = dosis, y = jurisdiccion)) +
  labs(title = "b) Barras horizontales") + theme_bw()
```
...
```{r}
#| echo: false
ggplot(dosis_aplicadas) + geom_bar(aes(x = jurisdiccion)) +
  labs(title = "c) geom_bar cuenta filas: todas miden 1") + theme_bw()
```
...
```{r}
#| echo: false
ggplot(dosis_aplicadas) + geom_col(aes(x = "", y = dosis, fill = jurisdiccion)) +
  labs(title = "d) Una sola barra apilada", x = "") + theme_bw()
```
:::

### Ejercicio: encuentra el error (barras)
...
a.  El `fill = jurisdiccion` está **afuera** del `aes` pero `jurisdiccion` **es una columna**, no un color fijo. Las columnas van adentro del `aes`: `geom_col(aes(x = jurisdiccion, y = dosis, fill = jurisdiccion))`. (Es la misma regla del ejercicio de puntos, pero al revés.)

b.  Falta el `+` al final del primer `labs`. Sin él, `R` termina la gráfica ahí y luego ejecuta un segundo `labs` suelto que no está pegado a nada. Mejor todavía: junta los dos en uno solo, `labs(x = "Jurisdicción", y = "Dosis aplicadas")`.

c.  `"dosis"` entre comillas es el **texto** "dosis", no la columna. Como es el mismo texto para las tres filas, todas las barras salen iguales. Quítale las comillas.

d.  Éste **no da error**, y por eso es el peor: el `theme_bw()` va **después** del `theme()` y lo sobrescribe por completo, así que la rotación de 45 grados nunca se aplica. Los `theme_algo()` completos siempre van **antes** de los `theme()` con ajustes finos. Es el mismo comentario que aparece en el código de la sección de arriba.
:::
...
ggplot(datos_diabetes) + 
  geom_histogram(aes(x = chol, y = after_stat(density)), bins = 50,
                 fill = "#cef0ff",  color = "#1D1B1B") + 
  geom_density(aes(x = chol), color = "deepskyblue4",
               linetype = "dotted", linewidth = 1) +
  labs(
...
ggplot(datos_diabetes) + 
  geom_histogram(aes(x = chol, y = after_stat(density)), binwidth = 10,
                 fill = "#cef0ff",  color = "#1D1B1B") + 
  geom_density(aes(x = chol), color = "deepskyblue4",
               linetype = "dotted", linewidth = 1) +
  labs(
...
#Graficación con barras de ancho 0.05
ggplot(datos_diabetes) + 
  geom_histogram(aes(x = height, y = after_stat(density)), color = "black",
               fill = "#fffff8", bins = 10) +
...
plotA <- ggplot(mis.datos) + geom_histogram(aes(x = A, y = (after_stat(count))/sum(after_stat(count))), fill = "tomato3", color = "black", binwidth = bw) +
  ylab("Proporción") +   theme(plot.background = element_rect(fill = "#fffff8"),
        panel.background = element_rect(fill = "#fffff8"))

plotB <- ggplot(mis.datos) + geom_histogram(aes(x = B, y = (after_stat(count))/sum(after_stat(count))), fill = "forestgreen", color = "black", binwidth = bw) +   theme(plot.background = element_rect(fill = "#fffff8"),
        panel.background = element_rect(fill = "#fffff8")) +
  ylab("Proporción")

plotC <- ggplot(mis.datos) + geom_histogram(aes(x = C, y = (after_stat(count))/sum(after_stat(count))), fill = "purple", color = "black", binwidth = bw) +
  ylab("Proporción") +   theme(plot.background = element_rect(fill = "#fffff8"),
        panel.background = element_rect(fill = "#fffff8"))

plotD <- ggplot(mis.datos) + geom_histogram(aes(x = D, y = (after_stat(count))/sum(after_stat(count))), fill = "deepskyblue3", color = "black", binwidth = bw) +
  ylab("Proporción") +   
  theme(plot.background = element_rect(fill = "#fffff8"),
        panel.background = element_rect(fill = "#fffff8"))
...
### Ejercicio a mano: construye el histograma con lápiz
...
#| eval: false
ggplot(edades) +
  geom_histogram(aes(x = edad), binwidth = 10, boundary = 20,
...
ggplot(edades) +
  geom_histogram(aes(x = edad), binwidth = 10, boundary = 20,
                 fill = "deepskyblue3", color = "white") +
  labs(x = "Edad (años)", y = "Personas",
       title = "Con intervalos de 10 años") +
  theme_bw()
```
...
#| echo: false
ggplot(edades) +
  geom_histogram(aes(x = edad), binwidth = 5, boundary = 20,
                 fill = "tomato3", color = "white") +
  labs(x = "Edad (años)", y = "Personas",
       title = "Con intervalos de 5 años: misma gente, otra historia") +
  theme_bw()
```
...
### Ejercicio: encuentra el error (histogramas)
...
a.  Un histograma **calcula solo** el eje `y` (es el conteo). No hay que dárselo, y menos la misma variable de la `x`. Correcto: `geom_histogram(aes(x = edad))`.

b.  `binwidth` y `bins` son dos formas distintas de decir lo mismo (`binwidth` es el ancho de cada barra, `bins` es cuántas barras quieres). Si le das las dos, `R` te avisa que va a ignorar una. Deja sólo `binwidth = 5`.

c.  `porcentaje` no es una columna de la base ni una variable calculada. Para eso `ggplot2` tiene variables internas que se piden con `after_stat`: `aes(x = edad, y = after_stat(count) / sum(after_stat(count)))`. (Si te topas con código viejo de `ggplot2` vas a ver esto mismo escrito como `..count..` y `..density..`, entre puntos. Esa forma todavía funciona pero ya está marcada como obsoleta: si la usas, `R` te va a llenar la consola de advertencias.)

d.  `jurisdiccion` es **texto**, y un histograma sólo funciona con variables **numéricas**: no se puede preguntar cuántas jurisdicciones caen entre "Centro" y "Norte". Para contar categorías se usa `geom_bar`, que es justo lo que viste en el ejercicio anterior.
:::

### Ejercicio con apoyo de una IA (histogramas)
...
### Ejercicio a mano: dibuja la trayectoria
...
#| echo: false
plot_grid(
  ggplot(brote) + geom_line(aes(x = semana, y = casos)) +
    labs(title = "a) sólo línea") + theme_bw(),
  ggplot(brote) + geom_line(aes(x = semana, y = casos)) +
    geom_point(aes(x = semana, y = casos)) +
    labs(title = "b) línea + puntos") + theme_bw(),
  ncol = 2)
```
...
brote_desordenado <- brote[c(3, 1, 5, 2, 6, 4), ]
plot_grid(
  ggplot(brote_desordenado) + geom_line(aes(x = semana, y = casos)) +
    labs(title = "c) geom_line: ordena por x") + theme_bw(),
  ggplot(brote_desordenado) + geom_path(aes(x = semana, y = casos)) +
    labs(title = "d) geom_path: respeta el orden") + theme_bw(),
  ncol = 2)
```

> **¿Y esto para qué me sirve?** Para series de tiempo casi siempre quieres `geom_line`. `geom_path` sirve cuando el orden *sí* es la información: por ejemplo para dibujar la trayectoria de un huracán, el recorrido de una brigada, o una gráfica donde el eje `x` es la prevalencia y el `y` la mortalidad y quieres ver cómo se movió un país año con año.
:::

### Ejercicio: encuentra el error (trayectorias)
...
a.  Falta la `y`. Sin ella `ggplot2` no sabe qué altura darle a la línea. Correcto: `aes(x = semana, y = casos)`.
...
c.  `geom_hline` dibuja líneas **horizontales** y por eso su argumento es `yintercept`. Para una línea **vertical** en la semana 3 se usa `geom_vline(xintercept = 3, linetype = "dashed")`.

d.  `ylim` necesita **dos** números, el mínimo y el máximo: `ylim(0, 100)`. Si sólo quieres fijar el piso y que `R` decida el techo, usa `expand_limits(y = 0)` o `ylim(0, NA)`.
...
### Antes de correr nada: dibújalas
...
### Ejercicio de cierre con apoyo de una IA
...
-   [Graficación con `ggplot2` parte 2](graficas-ggplot2-2.qmd).
