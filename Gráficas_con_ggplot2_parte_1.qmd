...
c.  `porcentaje` no es una columna de la base ni una variable calculada. Para eso `ggplot2` tiene variables internas que se piden con `after_stat`: `aes(x = edad, y = after_stat(count) / sum(after_stat(count)))`. (En las notas de arriba verás la forma vieja `..density..` y `..count..`, que todavía funciona pero ya está marcada como obsoleta en las versiones actuales de `ggplot2`.)
