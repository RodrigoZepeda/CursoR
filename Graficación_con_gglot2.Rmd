...
2. Dentro del `aes` puedes también aplicar funciones a las variables. Por ejemplo si queremos graficar índice de masa corporal ($\text{peso (kg)} / (\text{altura (m)}^2)$) podemos calcular el IMC en el mismo `aes` haciendo: `... aes(x = (weight * 0.453592) /(height * 0.0254)^2, ...) ...`. Donde el factor `0.453592` es para convertir de libras a kilos y `0.0254` de pulgadas a metros.  Grafica en la variable x el índice de masa corporal (IMC) y en la variable `y` el radio de colesterol definido como el nivel de colesterol `chol` entre el colesterol de alta densidad `hdl`. Obtén una gráfica como esta:  
...
3. Descarga y lee la base de [Dosis aplicadas de BCG en el IMSS](http://datos.imss.gob.mx/dataset/informacion-en-salud/resource/62b158dd-3b17-470e-b7c3-dd3b3cfd9f34). Crea una nube de puntos donde en el eje $x$ esté el cambio en dosis de `2002` a `2003` y en el eje $y$ esté el cambio en dosis de `2003` a `2004`. Es decir el eje $x$ representa cuánto fue el aumento (o disminución) en dosis en `2003` y el eje `y` representa el mismo cambio pero en `2004`. 
...
> Para este ejercicio puedes usar cualquier base(s) de datos que tú desees. Si no se te ocurre ninguna te recomendamos usar `ile-2019-2021.csv` que contiene los registros de interrupciones legales del embarazo en Ciudad de México o bien `linea-mujeres.csv` que contiene información sobre las llamadas realizadas a la Línea Mujeres de la Ciudad de México hasta noviembre del 2020. 
...
El ejercicio consiste en realizar dos gráficas de distinto tipo (_ej_ una de barras y una de tendencia) analizando elementos de tu base de datos. 
  ...
  + En esta otra liga puedes encontrar otros paquetes de graficación que se apoyan sobre `ggplot2` para hacer más gráficas: [https://exts.ggplot2.tidyverse.org/gallery/](https://exts.ggplot2.tidyverse.org/gallery/). 
