...
`ggplot` trabaja con `dataframes`; los `dataframes` son el formato preferido para tener tablas de datos. Probemos leer los datos de `Ejemplo1.csv`. Para ello recuerda que el comando es `read.csv` seguido del directorio del archivo: 
...
Dicha base de datos simulados ya se encuentra en un formato legible lista para ser graficada por lo que no necesitamos limpiarla (que veremos en otra sección). 
...
Las gráficas cuentan con varios temas (`themes`) con diseños predefinidos para cambiar su imagen. Prueba los distintos: `theme_classic`, `theme_bw`, `theme_gray`, `theme_linedraw`, `theme_light`, `theme_minimal`, `theme_dark` y `theme_void`.  
...
ggplot(Example1) + 
  geom_histogram(aes(x = Peso, y = ..density..), bins = 30, 
                 fill = "#EC4D37",  color = "#1D1B1B") + 
  geom_density(aes(x = Peso), color = "orange", size = 1)
```
...
ggplot(Example1) + 
  geom_histogram(aes(x = Peso, y = ..density..), bins = 30, 
                 fill = "#EC4D37",  color = "#1D1B1B") + 
  geom_density(aes(x = Peso), color = "black", 
...
ggplot(mis.datos) + geom_histogram(aes(x = x, y = ..density..), fill = "tomato3", color = "black", binwidth = 0.75) 
```
...
ggplot(mis.datos) + geom_histogram(aes(x = x, y = ..density..), fill = "tomato3", color = "black", binwidth = 1.2) 
```
...
plotB <- ggplot(mis.datos) + geom_histogram(aes(x = B, y = ..density..), fill = "forestgreen", color = "black", binwidth = bw) +
  ylab("Proporción")
...
Utiliza el siguiente comando para restar los valores de la columna de `A` a la base de datos:

```{r, echo = FALSE}
grid.arrange(plotA, plotB, plotC, plotD, top = "Todos los histogramas que requieres")
```
...
Nota que hay gráficas donde el porcentaje supera el 100% esto es porque `R`, de inicio, combina los años `2010` y `2015` sumando los valores. Podemos ver esto con el comando `fill` diciéndole a `R` que `Periodo` no lo lea como número sino como `String` (de otra forma buscará colorearlo con el color `2010` y`2015` los cuales no se distinguen:
...
ggplot(seguro.pop) + 
  geom_bar(aes(x = Entidad, weight = PorcentajeAfiliada, 
               fill = as.factor(Periodo)), position = "dodge") +
  coord_flip() 
```
...
Continuaremos con los datos de `Zika` pero esta vez utilizaremos los datos de México y sólo utilizaremos casos confirmados:
