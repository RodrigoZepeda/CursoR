...
options(repos="https://cran.itam.mx/ ")
```
<!--
...
Mi sugerencia personal es que escribas todo lo que haces en el `Script` y que sólo utilices la consola para verificar valores. De esta manera podrás almacenar todas las instrucciones ejecutadas y volver a ellas cuando se requieran. Asegúrate de que al guardar tu `Script`, éste tenga una codificación UTF-8 (Sobre todo si trabajas en Windows). 

```{r,  fig.cap = "El formato de codificación UTF-8, permite representar todos los caracteres que se usan en todos los idiomas. ", cache=TRUE, echo = FALSE,  message=FALSE}
knitr::include_graphics('images/Save_with_encoding.png')
```

Por último te sugiero utilizar `#` gatos para comentar tu código. Así, el código anterior lo podrías ver en la consola como:
