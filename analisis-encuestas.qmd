...
salud_adultos_svy %>%
  ...
  mutate(Proporción = paste0(
    scales::percent(prop, accuracy = 2),
    " [",
  ...
  select(-N, -n) %>%
  pivot_wider(
    id_cols = `edad_name`, names_from = sexo_name,
    values_from = c(Proporción, `Total (muestra)`)
  ) %>%
  select(
    edad_name, `Proporción_Hombre`, `Total (muestra)_Hombre`,
...
Usaremos `srs.bootstrap.sample` del paquete `surveybootstrap`. La función es
directa: le das tu base y cuántas réplicas quieres.
...
¿Qué hay dentro de cada réplica? No son los datos, sino **instrucciones para
rearmarlos**:
...
-   **`index`**: qué renglones de tu base entran en esta muestra bootstrap. Se
    eligen **con reemplazo**, así que un mismo renglón puede salir varias veces (o
    ninguna). Ésa es toda la magia del bootstrap.
-   **`weight.scale`**: un factor por el que hay que multiplicar los ponderadores
    originales. En el bootstrap `srs` vale 1.
...
::: callout-important
## Lo que `srs` supone (y por qué hay que decirlo)

El `srs` de `srs.bootstrap.sample` significa *simple random sample*: la función
remuestrea renglones **como si la ENSANUT fuera una muestra aleatoria simple**,
sin tomar en cuenta los estratos ni las UPM.

Es la versión más fácil de entender, y por eso la usamos para aprender el
mecanismo. Pero tenlo presente: cuando hay conglomerados, la gente de una misma
manzana se parece entre sí y aporta menos información de la que aparenta, así que
el error estándar "verdadero" suele ser **mayor**.

El mismo paquete trae `rescaled.bootstrap.sample`, que sí respeta los
conglomerados y se usa igual (también devuelve `index` y `weight.scale`):
...
::: callout-note
Fíjate en que el `for` no tiene nada de especial: es el mismo ciclo que viste en
el [capítulo de ciclos](ciclos.qmd). Lo único que cambia en cada vuelta son los
renglones que entran y sus ponderadores.
:::
