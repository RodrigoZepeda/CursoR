...
---
::: callout-warning
Si no lo has leído aún ve la [parte 1 de regresiones](regresiones-1.qmd)
:::
...
```{r}
#| warning: false
# Copia local de los datos de ceb.dat de https://data.princeton.edu/wws509/datasets/
niños <- read_table("datasets/niños.txt", skip = 1, col_names = F)
...
```{r}
niños <- niños %>%
  mutate(edf = factor(educ,
    levels = c("none", "lower", "sec+", "upper"),
    ordered = T
  ))
poisson_reg() %>%
  set_engine("glm") %>%
...
```{r}
# Copia local de los datos de cuse.raw de https://data.princeton.edu/wws509/datasets/
anticonceptivos <- read_table("datasets/anticonceptivos.txt",
  skip = 0, col_names = F
) %>% uncount(X5)

colnames(anticonceptivos) <- c(
  "age", "education", "wants_more_children",
  "using_contraceptive"
)
...
1.  Construye tres modelos distintos para predecir diabetes **sin usar la variable de glucosa** (`PlasmaGlucose`). Usaremos la misma base `diabetes` de esta sección, que ya tiene el desenlace `Diabetic` (0 = no, 1 = sí):

```{r}
diabedatos <- diabetes %>%
  select(-PlasmaGlucose) # quitamos la glucosa a propósito

# Así queda
dim(diabedatos)
table(diabedatos$Diabetic)
```

::: callout-note
Este ejercicio antes usaba la base `PimaIndiansDiabetes` del paquete `mlbench`.
Esa base **ya no viene incluida en el paquete** (la quitaron), así que la
cambiamos por una que sí está disponible. Si te topas con código viejo que la
usa, ahora ya sabes por qué no le funciona.
:::

::: callout-tip
Quitar la glucosa hace el problema mucho más difícil **a propósito**: la glucosa
es prácticamente la definición del desenlace, así que un modelo que la incluya
va a predecir casi perfecto sin enseñarte nada. Lo interesante es ver cuánto se
puede predecir con lo demás.
:::
...
```{r}
library(readxl)
# Copia local. Originalmente venía del Epi R Handbook, pero esa liga ya da 404.
campylobacter_germany <- read_excel("datasets/campylobacter_germany.xlsx")
```
