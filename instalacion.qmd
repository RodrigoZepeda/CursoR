...
g <- arrangeGrob(
  ...
  hist_right + geom_vline(xintercept = -0.5, linetype = "dashed", color = "forestgreen"),
  ncol = 2, nrow = 2, widths = c(5, 2), heights = c(2, 5),
...
- En el caso de **Mac** selecciona `Download R for macOS`. Ahí verás **dos** instaladores y tienes que elegir el que corresponde al procesador de tu computadora:

  + Si tu Mac es de las nuevas (procesador **Apple Silicon**: M1, M2, M3, M4 o más reciente) elige ``r paste0("R-", R.Version()$major,".",R.Version()$minor,"-arm64.pkg")``.
  + Si tu Mac es de las viejitas (procesador **Intel**) elige ``r paste0("R-", R.Version()$major,".",R.Version()$minor,"-x86_64.pkg")``.

  Para saber cuál tienes, ve al menú de la manzanita ` > Acerca de esta Mac` y fíjate en el renglón de `Chip` (Apple Silicon) o `Procesador` (Intel). En Mac puede que necesites instalar adicionalmente [XQuartz](https://www.xquartz.org) (según tu versión de Mac). Si tu Mac es una versión antigua (mayor a 6 años), sigue las instrucciones específicas de `CRAN`.
...
:::{.callout-tip}
Si tienes problemas para instalar puedes usar [Posit Cloud (antes RStudio Cloud)](https://posit.cloud), que corre `R` y `RStudio` desde el navegador sin instalar nada.
:::

### Rtools (Windows)

`RTools` es una herramienta para poder instalar paquetes de `R`. Algunas versiones de Windows lo requieren. Entra [al sitio de RTools](https://cran.r-project.org/bin/windows/Rtools/), elige la opción que corresponde a tu versión de `R` (puedes verla con `R.version`) y descarga el instalador correspondiente.

::: {.callout-important}
La versión de `RTools` **debe coincidir** con la de tu `R`. Como estas notas se hicieron con `r R.Version()$version.string`, aquí te tocaría ``r paste0("RTools", R.Version()$major, substr(R.Version()$minor, 1, 1))``. La regla es sencilla: si tu `R` es la versión `4.5.x` necesitas `RTools45`, si es `4.6.x` necesitas `RTools46` y así sucesivamente.
:::
...
Para descargar `RStudio` ve a [https://posit.co/download/rstudio-desktop/](https://posit.co/download/rstudio-desktop/). La página tiene dos pasos: el paso `1` es instalar `R` (que ya hiciste) y el paso `2` es el botón de descarga de `RStudio Desktop`, que detecta solo tu sistema operativo. Si quieres elegir a mano tu plataforma (`Windows`, `macOS` ó tu sabor de `Linux` preferido) baja tu pantalla hasta la tabla `All Installers and Tarballs`. Siempre usa la versión `Desktop` gratuita (la de paga se llama `RStudio Workbench` y no la necesitamos). Una vez descargado el archivo, ábrelo y sigue las instrucciones que aparecen en pantalla.

::: {.callout-note}
`RStudio` se numera por año, no como `R`. Las versiones se ven así: `2026.04.1+512` (año, mes y número de parche) y además tienen nombre de flor. No te preocupes por cuál te toca: la página siempre te ofrece la más reciente y ésa es la que queremos.
:::
...
* [Introducción a R](introduccion-a-r.qmd)
