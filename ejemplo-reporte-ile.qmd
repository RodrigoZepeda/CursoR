---
title: "Ejemplo de reporte: interrupción legal del embarazo y COVID-19"
aliases:
  - "/EjemploAnalisis.html"
---
...
```{r}
# Lectura de la base
ile.data <- read_csv("datasets/ile_completo.csv", show_col_types = FALSE)

# Lectura del diccionario de datos
diccionario.data <- read_csv("datasets/diccionario-de-datos-ile.csv",
...
ile.data %>%
  filter(!is.na(año)) %>%
  ggplot() +
  geom_histogram(aes(x = edad, y = after_stat(density), fill = as.character(año)),
    bins = 25, color = "white"
  ) +
  ...
  ggtitle(paste0(
    "Distribución de la edad en acceso a ILE en clínicas",
...
ile.data %>%
  ...
  geom_spline(aes(x = `fingreso`, y = n, color = "Tendencia"),
    nknots = 5,
    size = 2
  ) +
  theme_minimal() +
...
ile.data %>%
  ...
  pivot_wider(
    id_cols = entidad, names_from = Pandemia,
    values_from = `Entidad (%)`
  ) %>%
